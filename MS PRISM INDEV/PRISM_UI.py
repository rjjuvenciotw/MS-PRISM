...
# Variáveis globais para conexão, criptografia.
HOST = 'localhost'
PORT = 5555
cliente_socket = None
conectado = False
KEY = b'ABCChaveSecretaCom32BytesExatas!'  # Chave de 32 bytes para AES-256

# Variáveis globais para elementos de interface.
janela = None
btn_opcao = None
...
def Ui_interface_grafica():
    ...
    # Frame para os botões no cabeçalho
    ...
    btn_opcao = ctk.CTkButton(botao_frame, text="Conectar", fg_color="White",
                                    hover_color="#275EDF", border_color="#505050",text_color="black",
                                    corner_radius=15, border_width=0, width=70,
    ...
    frame_esquerdo = ctk.CTkFrame(janela, fg_color="#0D0C10", width=500,  # Aumente a largura aqui
    ...
    messages_container = ctk.CTkScrollableFrame(frame_esquerdo, fg_color="#0D0C10", width=350,)
    messages_container.pack(fill="both", expand=True, padx=5, pady=5)

    
    # 5 - Frame Direito(Ler e enviar mensagens.)
    frame_direito = ctk.CTkFrame(janela, width=400, fg_color="#0D0C10",
    ...
    titulo_painel = ctk.CTkLabel(frame_direito, text="Digite o assunto:", text_color="gray", 
                                font=ctk.CTkFont("Times", 16))
    titulo_painel.pack(anchor="w", padx=10, pady=10)

    # 6 - Entrada de dados de mensagens.
    entrada_assunto = ctk.CTkEntry(frame_direito, placeholder_text="Assunto:",
                                  width=1000, height=40, border_width=1,
                                  corner_radius=10, fg_color="#242328",
                                  font=ctk.CTkFont(size=15))
    entrada_assunto.pack(anchor="w", padx=10, pady=5)

    # Criação do frame para o label e o botão
    frame_label_botao = ctk.CTkFrame(frame_direito, fg_color="transparent",)
    frame_label_botao.pack(fill='x', padx=10, pady=10)

    # Configuração das colunas do grid no frame_label_botao
    frame_label_botao.columnconfigure(0, weight=5)  # Coluna do label
    frame_label_botao.columnconfigure(2, weight=0)  # Coluna do botão

    # Label "Digite sua mensagem:"
    titulo_painel = ctk.CTkLabel(frame_label_botao, text="Digite sua mensagem:", text_color="gray",
                                font=ctk.CTkFont("Times", 16,))
    titulo_painel.grid(row=0, column=0, sticky='w')

    # Botão "Limpar"
    btn_limpar = ctk.CTkButton(frame_label_botao, text="Limpar tudo", fg_color="white", text_color="black",
                           hover_color="red", corner_radius=0, command=limpar_campos, width=30)
    btn_limpar.grid(row=0, column=1, sticky='e', padx=10)
...
def encrypt_message(message):
    ...
    try:
        ...
        iv = cipher.iv
        encrypted_mensagem = cipher.encrypt(pad(message.encode('utf-8'), AES.block_size))
        mensagem_completa = iv + encrypted_mensagem
        return mensagem_completa.hex()
...
def decrypt_message(encrypted_hex):
    if not encrypted_hex:
        print("Erro: Mensagem criptografada vazia")
        return None
        
    try:
        encrypted_bytes = bytes.fromhex(encrypted_hex)
        iv = encrypted_bytes[:AES.block_size]
        encrypted_mensagem = encrypted_bytes[AES.block_size:]
        cipher = AES.new(KEY, AES.MODE_CBC, iv)
        decrypted = unpad(cipher.decrypt(encrypted_mensagem), AES.block_size)
...
def pagina_sobre():
    ...
    alerta.iconbitmap("MIDIA/Group-1.ico")
...
def preview_mensagem(assunto, mensagem):
    ...
    data = ctk.CTkLabel(frame_mensagem, text=data_formatada, text_color="gray", font=ctk.CTkFont("Arial", 10))
    data.pack(side="right", padx=10, pady=5)
...
def notificacao_alerta():
    pygame.mixer.init()
    pygame.mixer.music.load('MIDIA/not_alert.mp3')
    pygame.mixer.music.set_volume(1.0)  # Volume máximo
    pygame.mixer.music.play()

def som_pagar_msg():
    pygame.mixer.init()
    pygame.mixer.music.load('MIDIA/not_apagar.mp3')
    pygame.mixer.music.set_volume(1.0)  # Volume máximo
    pygame.mixer.music.play()
